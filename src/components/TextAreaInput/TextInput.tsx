...
interface Props {
	...
	max?: string; // Optional for future use
}
...
const TextInput = ({
	...
	return (
		<div className='flex flex-col gap-1'>
			{label && (
				<label
					className={`flex items-center gap-1 text-xs sm:text-sm font-medium text-white ${labelClassName}`}
					htmlFor={id}
				>
			...
			<div className='relative'>
				...
				{showPasswordIcon && (
					<span
						className={`absolute top-[23px] lg:top-7 right-4 transform -translate-y-1/2 cursor-pointer transition-[.3] ${passwordIconClassname}`}
						onClick={togglePasswordVisibility}
					>
						{showPassword ? (
							<FaEye className='text-white/80 text-sm' />
						) : (
							<FaEyeSlash className='text-white/80 text-sm' />
						)}
					</span>
				)}
