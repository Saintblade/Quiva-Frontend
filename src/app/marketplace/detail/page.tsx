...
//       price: comic.price || 0,
//       nftDetails: comic.nftDetails,
//       tokenId: comic.nftDetails?.tokenId,
//       issueDetails: {
//         creators: comic.creatorId?.username || "Unknown",
//         pages: comic.totalPages || 0,
//         publisher: "Independent",
...
//     if (!transformedComic.tokenId || !currentComic?.creatorId?.walletAddress) {
//       alert('Missing NFT information')
//       return
//     }
...
//         seller: currentComic.creatorId.walletAddress,
//         amount: BigInt(1), // Default to 1 copy
//         pricePerToken: parseEther(transformedComic.price.toString()),
//       })
//     } catch (error) {
...
//       {transformedComic.publishType === 'nft' && (
//         <PurchaseNFTModal
...
//           price={parseEther(transformedComic.price.toString())}
//           tokenId={BigInt(transformedComic.tokenId || 0)}
//           sellerAddress={currentComic?.creatorId?.walletAddress || ''}
//           limitedEdition={`${transformedComic.nftDetails?.currentSupply || 0}/${transformedComic.nftDetails?.maxSupply || 100}`}
//           onPurchaseSuccess={handlePurchaseNFT}
...
function Page() {
  ...
  return (
    <>
      ...
      {/* Show alert if user tries to purchase un-minted NFT
      {transformedComic.publishType === 'nft'  && isPurchaseModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gray-900 rounded-2xl max-w-md w-full border border-white/10 p-6">
            <h3 className="text-xl font-bold text-white mb-4">NFT Not Ready</h3>
            <p className="text-white/60 mb-6">
              This NFT is still being minted. Current status: <span className="text-yellow-500">{transformedComic.nftDetails?.mintStatus || 'pending'}</span>
            </p>
            <button
              onClick={handleClosePurchaseModal}
              className="w-full bg-yellow-600 hover:bg-yellow-700 text-black font-bold py-3 rounded-full transition-all"
            >
              Got it
            </button>
          </div>
